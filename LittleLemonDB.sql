...
CREATE TABLE `orders` (
  ...
  KEY `order_delivery_status_id_fk_idx` (`OrderDeliveryStatusID`),
  CONSTRAINT `menu_id_fk` FOREIGN KEY (`MenuID`) REFERENCES `menu` (`MenuID`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `order_delivery_status_id_fk` FOREIGN KEY (`OrderDeliveryStatusID`) REFERENCES `orderdeliverystatus` (`OrderDeliveryStatusID`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2025-01-15 20:38:06
